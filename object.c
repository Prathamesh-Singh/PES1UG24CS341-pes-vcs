...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    // 1. Build header string: "blob 42\0", "tree 128\0", or "commit 256\0"
    const char *type_str = (type == OBJ_BLOB)   ? "blob"   :
                           (type == OBJ_TREE)   ? "tree"   : "commit";
    char header[64];
    // snprintf writes the null terminator; hlen includes it (+1)
    ...
    // 4. Deduplication — if already stored, nothing to do
    ...
    // 6. Build final object path and a temp path in the same directory
    char path[300];
    object_path(id_out, path, sizeof(path));
    char tmp_path[316];
    snprintf(tmp_path, sizeof(tmp_path), "%s/%.2s/tmp_XXXXXX", OBJECTS_DIR, hex);

    // 7. Write to a temp file using mkstemp for safety
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    // 3. Integrity check — recompute hash and compare to *id
    ObjectID computed;
    ...
    *data_out = malloc(*len_out + 1); // +1 for safety null terminator
    ...
    ((char *)*data_out)[*len_out] = '\0'; // safe null terminator
